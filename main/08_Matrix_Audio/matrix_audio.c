...
static void audio_ui_apply(const audio_state_t *st) {
   ...
   * ======================= */
  example_ui_t *b = ui.base;
  const esp_err_t style_ret =
      (st->audio_init_ret != ESP_OK) ? st->audio_init_ret : st->audio_open_ret;
   ...
   * 2. Basic Text
   * ======================= */
  const bool ok = (st->audio_init_ret == ESP_OK) &&
                  (st->audio_open_ret == ESP_OK) &&
                  (st->audio_read_ret == ESP_OK);
  ...
  snprintf(b->line4_text, sizeof(b->line4_text), "VOL:%u",
           (unsigned)st->audio_out_vol);
...
static void audio_data_update(lv_timer_t *t) { audio_ui_apply(&audio_state); }
